...
The registry enumerates **148 capabilities** against baseline `claude-code-2.1.x (mid-2026)`: **80 full**, **18 partial**, **41 degraded-noop**, **9 not-supported**. 5 entries are safety-relevant (marked ⚠) — a divergence where a project's restriction is not enforced and is therefore reported prominently. Unknown inputs outside this registry are not counted here: they are unassessed by definition and degrade safely at runtime (plan §2.4).
