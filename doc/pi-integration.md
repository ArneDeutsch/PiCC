...
> **Status:** Contract record for the coordinated Pi 0.82.0 suite
> (`@earendil-works/pi-agent-core`, `pi-ai`, `pi-coding-agent`, and `pi-tui`). `package.json` pins
> every direct declaration to exactly 0.82.0; strict admission verifies every lockfile and installed
...
| Env & exec | `pi.exec(cmd, args, { signal, timeout })` for git/hook commands; hooks additionally need shell execution via `node:child_process` (stdin JSON contract Pi's exec doesn't cover: we use `spawn` directly). Pi 0.82 Bash factories default to exposing `PI_SESSION_ID`, `PI_SESSION_FILE`, `PI_PROVIDER`, `PI_MODEL`, and `PI_REASONING_LEVEL`; PiCC passes `exposeSessionEnvironment:false`, then overlays sanitized `setting.env` and the required project-root `CLAUDE_PROJECT_DIR`. |
