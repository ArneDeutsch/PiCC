...
ambiguous path uses `ctx.abort()` and the provider guard. Pi 0.83 persists either PiCC-owned
pre-commit cutoff mechanism as a physical `aborted` response. That response continues the
already-authorized checkpoint only when no resume is active, the checkpoint generation remains
active, and its controller phase is exactly `awaiting-settlement`. `checkpointAbortRequested`
records only whether PiCC selected physical abort rather than terminate; it records neither logical
failure nor eligibility for this exception. A genuine `pending` or `error` at that boundary becomes
pre-commit operational exhaustion with recoverable manual-compaction guidance. After the synthetic
resume begins, any unsuccessful terminal outcome is post-commit and terminal.
