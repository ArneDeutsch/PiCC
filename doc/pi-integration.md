...
| PiCC control commands | `pi.registerCommand(name, { description, handler })` for interactive routing; the shared `pi.on("input")` handler intercepts the same names in non-interactive modes; registered handlers get `ExtensionCommandContext`; keep the command map, input interceptor, dispatcher, and reserved names in `src/index.ts` synchronized |
...
| Control output, checkpoint records, and status notifications | `ctx.ui.notify` for TUI status; `pi.appendEntry` records entries across modes, while `pi.registerEntryRenderer` supplies their TUI presentation |
