...
| Main-session tool and MCP exposure | Post-load `pi.registerTool` of a new name refreshes Pi's registry, and a snippet-less tool leaves the base system prompt byte-identical. Ordinary sessions may auto-activate newly registered global MCP names. Selected sessions instead reconcile their immutable policy against Pi's observable `getActiveTools()`/`setActiveTools()` set before provider admission and at every turn; this is an intersection with host-visible active intent, not an atomic registry transaction or proof of third-party mutation provenance, so read/evaluate/write uncertainty fails selected admission closed. Selected MCP definitions still use the API-global registration seam but route through a generation-fenced selected adapter; dispatch-only scopes build local custom-tool definitions for `createAgentSession`. Remote recovery reuses original proxies, fixed resource definitions, and immutable catalogs rather than registering replacements. No discovered proxies means no proxy schemas; no advertised resource capability in the settled initial snapshots means neither fixed resource schema. Pinned against the real Pi dist in `test/mcp-registration.test.ts`. |
...
post-commit and terminal except the exact authenticated selected-branch `aborted` response followed by
settlement of that same run. That exception permits safe in-session recovery only in the TUI; print and
JSON use terminal `retrieve-and-relaunch`, while live RPC is process-terminal `restart-process`.
