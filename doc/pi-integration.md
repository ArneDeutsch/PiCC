...
| Env & exec | PiCC resolves its Git executable from the absolute `PICC_GIT` override or PATH. Startup/worktree Git uses `node:child_process.execFile` with sanitized inherited environment because `pi.exec` options do not accept `env`; hooks use `spawn` for their shell/stdin JSON contract. Project subprocess environments first remove launcher/host-only ambient context, including Pi's `AI_AGENT` (all casing aliases on Windows), then apply explicit/configured values and the required project-root `CLAUDE_PROJECT_DIR`. Pi Bash factories default to exposing `PI_SESSION_ID`, `PI_SESSION_FILE`, `PI_PROVIDER`, `PI_MODEL`, and `PI_REASONING_LEVEL`; PiCC passes `exposeSessionEnvironment:false`. Direct Bash traverses the composed `user_bash` event chain once and delegates to Pi's local operations without a model turn. Embedded-launch suppression clears before execution; Windows pins Git Bash when available, while other platforms retain Pi's normal local backend. |
| JSON/RPC streaming | Pi's official `message_update` wire records carry delta content only. For each record, clients inspect `assistantMessageEvent`, group content by its `contentIndex`, and branch on its `type`: append the event's `delta` for `text_delta` and `thinking_delta`, or buffer it as a JSON-argument fragment for `toolcall_delta`; a `toolcall_end` event's `toolCall` is authoritative for the completed call. PiCC provides no cumulative-content adapter. Top-level usage remains cumulative for the current assistant response, while `message_start` and `message_end` are authoritative message boundaries. Lifecycle, command acknowledgement, cancellation, and PiCC custom-entry records retain their established shapes. |
