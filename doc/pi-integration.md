...
| PiCC control commands | `pi.registerCommand(name, { description, handler })` for interactive routing; the shared `pi.on("input")` handler intercepts the same names in non-interactive modes; registered handlers get `ExtensionCommandContext`; `CONTROL_COMMANDS` in `src/index.ts` owns the member set |
