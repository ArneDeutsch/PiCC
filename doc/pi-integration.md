...
| Control commands: `/doctor`, `/usage`, `/quota`, `/skills`, `/agents` | `pi.registerCommand(name, { description, handler })` for interactive routing; the shared `pi.on("input")` handler intercepts the same names in non-interactive modes; registered handlers get `ExtensionCommandContext` |
