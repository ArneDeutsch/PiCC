...
| PiCC control commands | The command map in `src/index.ts` is the single owner of PiCC command registration and dispatch; reserved-name lookup consumes that map alongside Pi's built-in-name set. `pi.registerCommand(name, { description, handler })` provides interactive routing; the shared `pi.on("input")` handler parses the same map for admitted non-interactive input so recognized commands stay outside model context. Registered handlers get `ExtensionCommandContext` |
