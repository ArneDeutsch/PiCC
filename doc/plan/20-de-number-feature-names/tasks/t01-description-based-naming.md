...
- For a new run, case-fold names for portable collision comparison and also use exact Git/path checks. Before `EnterWorktree`, reject the candidate plan path both in the fetched target-default tree and in the current filesystem (including a dangling link), any existing `.claude/worktrees/<feature-slug>` filesystem entry, registered worktree, local `feature/<feature-slug>` ref, local harness `worktree-<feature-slug>` ref, or fetched remote `feature/<feature-slug>` ref. On any visible collision, state what was occupied, author/revalidate/recheck a more specific descriptive slug, and repeat the full title/slug/branch/plan announcement before invoking the tool; never increment a counter, overwrite, delete, reuse, or adopt the existing artifact. The normal identity announcement must include a concise user-facing disclosure that checks cover shared/fetched state but cannot eliminate simultaneous or disconnected same-slug races. Make this a hard presentation gate: immediately after build go, allow the required Phase 2 reference reads, then emit the complete announcement as user-visible prose (not hidden reasoning) before the first workspace/preflight command or `EnterWorktree`; it is informational, can share a response with subsequent tools, and does not wait for another reply.
...
- resume-before-allocation precedence, recovered slug/title validation, phase-aware heading/commit/title agreement (including plan/review commit title equality), verbatim-title human trust confirmation, anchor-target match, immutability, and a concrete end-to-end legacy override;
- exact worktree identity plus all four commit forms; title character/control/direct-copy constraints and quoted issue/PR command sites; workspace, templates, ticket/PR hand-off, fork compare URL with the concrete feature branch, CI, and cleanup;
- selected identity announcement as a hard user-visible post-reference-read/pre-workspace-command gate (without another reply), collision replacement reannouncement, user-facing residual-race disclosure, canonical GitHub numeric reference, and resolved `<pushRemote>` usage outside the no-`gh` degrade;
- exact frozen `<Title>` equality across ticket dedup/create/cache and quoted command sites; `CONTRIBUTING.md` uses `feature/<feature-slug>` and no obsolete checkout form.
...
- Dogfood one disposable ticketless run through descriptive identity announcement, workspace creation, and feature-spec creation, then abort without integration; record OS/shell, observed artifact names, cleanup, and unverified paths. Do not perform public GitHub writes for this smoke test.
