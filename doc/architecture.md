...
admission uses captured, preclassified policy/review authority, then materializes only inside its named
dispatch without later filesystem or Git acquisition; it never mutates ordinary resolution or widens
ordinary MCP sources or the parent inventory. Future plugin or explicit runtime/CLI adapters must
cross the same seam before their sources can be claimed as supported.
