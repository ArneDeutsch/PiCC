...
| Startup notice keeps appearing | `/compat suppress` (per-project, stored in `.claude/.picc/`). The MCP pending-approval notice deliberately survives suppression — approve the server(s) or list them in `disabledMcpjsonServers` to silence it. |
