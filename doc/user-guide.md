...
| MCP pending-approval notice at every startup | Approve selected servers with `enabledMcpjsonServers` in user settings or a clean, user-controlled, untracked local file, or list them in `disabledMcpjsonServers` to decline. Each UTF-16 code unit outside ASCII letters, digits, `_`, and `-` becomes `_`; an astral symbol therefore becomes `__`. One persisted named approval can match a differently named current or future server, so re-review aliases when project MCP names change. `/mcp` and `/doctor` provide bounded guidance; `enableAllProjectMcpServers` trusts all current and future servers and is not a shortcut |
