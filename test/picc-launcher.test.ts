...
`);
  ...
  it("commits a retained local marketplace and plugin through fresh production composition", async () => {
    ...
    expect(added.code, added.stderr.join("\n")).toBe(0); expect(addedText).toContain("Marketplace state changed"); expect(addedText).toContain("Source authority: kind=local-directory; display-anchor=opaque-sha256:");
