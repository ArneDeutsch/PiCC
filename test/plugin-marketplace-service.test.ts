...
import { canonicalJsonBytes, createProducerCodecRegistry, establishOwnedStateStore, type OwnedStateStore } from "../src/plugin-lifecycle/state-store.js";
import { createOwnedMarketplaceCodec, createOwnedMarketplaceSnapshotCodec, readOwnedAdmissionRecords } from "../src/plugin-lifecycle/admission.js";
...
function acquired(char = "a"): AcquiredMarketplaceSnapshot { const catalogBytes = Buffer.from(JSON.stringify({ name: "official", owner: { name: char === "a" ? "Example" : `Example ${char}` }, plugins: [{ name: "tool", source: { source: "npm", package: "tool", registry: "https://registry.npmjs.org" } }] })); const catalogDigest = `sha256:${createHash("sha256").update(catalogBytes).digest("hex")}` as Sha256; const executableCatalog = deriveExecutableMarketplaceCatalogProjection(catalogBytes, source.kind); if (executableCatalog === undefined) throw new Error("fixture projection"); return { target: { authorityKind: "catalog-only", marketplaceName: "official", snapshotId: snapshotId(catalogDigest, source.url), source, catalogDigest, executableCatalog, provenance: { adapter: "public-https-catalog", canonicalUrl: source.url } }, catalogBytes }; }
...
describe("PluginMarketplaceService", () => {
  ...
  it("persists a non-empty catalog executable projection through commit and fresh codec reload", async () => {
    const value = await fixture(); const catalogBytes = Buffer.from(JSON.stringify({ name: "official", owner: { name: "Example" }, allowCrossMarketplaceDependenciesOn: ["partner"], plugins: [{ name: "tool", source: { source: "npm", package: "tool", registry: "https://registry.npmjs.org" }, defaultEnabled: false, revision: "release-2", dependencies: [{ name: "base", marketplace: "partner", version: "^2" }] }] })); const catalogDigest = `sha256:${createHash("sha256").update(catalogBytes).digest("hex")}` as Sha256; const executableCatalog = deriveExecutableMarketplaceCatalogProjection(catalogBytes, source.kind)!; const exact: AcquiredMarketplaceSnapshot = { target: { authorityKind: "catalog-only", marketplaceName: "official", snapshotId: snapshotId(catalogDigest, source.url), source, catalogDigest, executableCatalog, provenance: { adapter: "public-https-catalog", canonicalUrl: source.url } }, catalogBytes }; const service = build(value, [], () => exact, "marketplace_projection_reload").service; const preview = await service.add("official", source); expect((await commit(service, preview)).ok).toBe(true); const registry = createProducerCodecRegistry([createOwnedMarketplaceCodec(value.store.profileKey as `profile-${string}`), createOwnedMarketplaceSnapshotCodec({ profileKey: value.store.profileKey as `profile-${string}`, artifactsRoot: value.store.artifactsRoot })]); if (!registry.ok) throw new Error(registry.message); const fresh = readOwnedAdmissionRecords(value.store, registry.value, undefined); expect(fresh.marketplaceSnapshots[0]?.executableCatalog).toEqual(executableCatalog); expect(fresh.marketplaceSnapshots[0]?.executableCatalog.declarations[0]).toMatchObject({ defaultEnabled: false, release: { kind: "revision", value: "release-2" }, dependencies: [{ name: "base", marketplace: "partner", version: "^2", itemIndex: 0 }], dependencyDeclaration: "complete" });
    const materializedValue = await fixture(); const localRoot = path.join(materializedValue.project, "catalog"); fs.mkdirSync(path.join(localRoot, ".claude-plugin"), { recursive: true }); fs.writeFileSync(path.join(localRoot, ".claude-plugin", "marketplace.json"), catalogBytes); const localSource = { kind: "local-directory", path: localRoot } as const; const adapter = createMarketplaceAcquisitionAdapter({ store: materializedValue.store }); const acquiredLocal = await adapter("official", localSource); if (!acquiredLocal.ok) throw new Error(acquiredLocal.message); const materializedService = build(materializedValue, [], () => acquiredLocal.value, "marketplace_materialized_projection_reload").service; const materializedPreview = await materializedService.add("official", localSource); expect((await commit(materializedService, materializedPreview)).ok).toBe(true); const materializedRegistry = createProducerCodecRegistry([createOwnedMarketplaceCodec(materializedValue.store.profileKey as `profile-${string}`), createOwnedMarketplaceSnapshotCodec({ profileKey: materializedValue.store.profileKey as `profile-${string}`, artifactsRoot: materializedValue.store.artifactsRoot })]); if (!materializedRegistry.ok) throw new Error(materializedRegistry.message); expect(readOwnedAdmissionRecords(materializedValue.store, materializedRegistry.value, undefined).marketplaceSnapshots[0]?.executableCatalog).toEqual(acquiredLocal.value.target.executableCatalog);
  });
  ...
  it("completes and rolls back marketplace pending state in genuinely fresh processes", async () => {
    const script = String.raw`
      ...
      import { deriveExecutableMarketplaceCatalogProjection } from "./src/util/plugin-marketplace-descriptor.ts";
  ...
  it("keeps visible unsupported declarations listable and refreshable/removable", async () => {
    const value = await fixture(); const bytes = Buffer.from(JSON.stringify({ name: "official", owner: { name: "Example" }, plugins: [{ name: "future-tool", source: { source: "future" } }] })); const catalogDigest = `sha256:${createHash("sha256").update(bytes).digest("hex")}` as Sha256; const executableCatalog = deriveExecutableMarketplaceCatalogProjection(bytes, source.kind)!; const unsupported: AcquiredMarketplaceSnapshot = { target: { ...acquired().target, catalogDigest, executableCatalog, snapshotId: snapshotId(catalogDigest, source.url) }, catalogBytes: bytes };
  ...
  it("binds catalog bytes to digest and derives source-context classification in the service", async () => {
    const value = await fixture(); const relativeBytes = Buffer.from(JSON.stringify({ name: "official", owner: { name: "Example" }, plugins: [{ name: "tool", source: "./tool" }] })); const catalogDigest = `sha256:${createHash("sha256").update(relativeBytes).digest("hex")}` as Sha256; const executableCatalog = deriveExecutableMarketplaceCatalogProjection(relativeBytes, source.kind)!; const contextual: AcquiredMarketplaceSnapshot = { target: { ...acquired().target, catalogDigest, executableCatalog, snapshotId: snapshotId(catalogDigest, source.url) }, catalogBytes: relativeBytes };
  ...
  it("keeps catalog credential canaries out of preview, recovery status, and receipt views", async () => {
    const value = await fixture(); const secret = "credential-user:credential-pass"; const bytes = Buffer.from(JSON.stringify({ name: "official", owner: { name: secret }, plugins: [] })); const catalogDigest = `sha256:${createHash("sha256").update(bytes).digest("hex")}` as Sha256; const executableCatalog = deriveExecutableMarketplaceCatalogProjection(bytes, source.kind)!; const next: AcquiredMarketplaceSnapshot = { target: { ...acquired().target, catalogDigest, executableCatalog, snapshotId: snapshotId(catalogDigest, source.url) }, catalogBytes: bytes }; let interrupted = false;
