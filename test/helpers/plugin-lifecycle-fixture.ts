...
export function createPluginLifecycleFixture(project: string, root: string): PluginLifecycleFixture {
  ...
  write(hookScript, [
    ...
    'const runIdValue=process.env.PICC_E2E_HOOK_RUN_ID;',
    'const hookRunId=typeof runIdValue === "string" && /^[0-9a-f]{32}$/.test(runIdValue) ? runIdValue : null;',
    'const piPidValue=process.env.PICC_E2E_PI_PID;',
    'const parsedPiPid=typeof piPidValue === "string" && /^[1-9][0-9]*$/.test(piPidValue) ? Number(piPidValue) : NaN;',
    'const hookPiPid=Number.isSafeInteger(parsedPiPid) && parsedPiPid > 0 ? parsedPiPid : null;',
    'const boundedString=value => typeof value === "string" && value.length <= 64 ? value : null;',
    'let input=""; process.stdin.setEncoding("utf8");',
    'process.stdin.on("data", chunk => input += chunk);',
    'process.stdin.on("end", () => {',
    '  let payload={}; try { payload=JSON.parse(input); } catch {}',
    `  fs.appendFileSync(${JSON.stringify(lifecycleTrace)}, JSON.stringify({event:boundedString(payload.hook_event_name),source:boundedString(payload.source),reason:boundedString(payload.reason),hookRunId,hookPiPid,hookChildPid:process.pid})+"\\n");`,
    `  if (payload.hook_event_name === "SessionEnd" && fs.existsSync(${JSON.stringify(shutdownMutationGate)})) {`,
