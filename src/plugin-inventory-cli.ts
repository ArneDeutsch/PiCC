...
function recoverySummaryLines(summary: unknown, completed: number): string[] {
  ...
  return [`Producer target/scope/action: ${safe(target, 160)}; ${safe(scope, 80)}; ${safe(value.action ?? "unknown", 80)}`, `Settings/declaration effect: ${settings === undefined ? "unchanged or unavailable" : settingsEffectText(settings)}`, `Record/generation/data/destructive effects: record=${safe(value.mutableRecordKey ?? "unchanged", 160)}; generation=${safe(value.generationId ?? "unchanged", 120)}; remove-declaration=${safe(value.removeDeclaration ?? "false", 20)}; remove-data=${safe(value.removeData ?? "false", 20)}; acknowledgement=${safe(value.acknowledgement ?? "none", 80)}`, `Participant effects: total=${participants.length}; ${participantText.join(", ") || "none"}`, `Producer consequences: total=${consequences.length}`, ...completeLines(consequences)];
}
