...
function boundedBasename(value: string): string { return safe(value.replaceAll("\\", "/").split("/").filter(Boolean).at(-1) ?? "local-source", 120); }
function publicHost(value: string): string { return value.replace(/[\u0000-\u001f\u007f]/gu, "").slice(0, 160); }
function sourceAnchorText(sourceValue: unknown): string {
  ...
  const localPath = typeof sourceRecord.path === "string" ? sourceRecord.path : typeof sourceRecord.localPath === "string" ? sourceRecord.localPath : undefined;
  const repository = typeof sourceRecord.repository === "string" ? sourceRecord.repository : typeof sourceRecord.repo === "string" ? sourceRecord.repo : undefined;
  let identity = localPath === undefined ? "" : `; local-name=${boundedBasename(localPath)}`;
  if (repository !== undefined && /^[A-Za-z0-9_.-]{1,100}\/[A-Za-z0-9_.-]{1,100}$/u.test(repository)) identity = `; repository=${repository}`;
  const rawUrl = typeof sourceRecord.url === "string" ? sourceRecord.url : undefined;
  if (rawUrl !== undefined) {
    try { const parsed = new URL(rawUrl); if (parsed.protocol === "https:" && parsed.username === "" && parsed.password === "") identity = `; host=${publicHost(parsed.host)}; remote-identity=${displayAnchor({ authority: parsed.host, path: parsed.pathname })}`; } catch {}
  }
  return `kind=${kind}${identity}; display-anchor=${displayAnchor(sourceValue)}`;
}
function settingsValueText(value: unknown): string {
  if (typeof value === "boolean" || value === null) return String(value);
  return sourceAnchorText(value);
}
function settingsSourceText(value: unknown): string { return typeof value === "string" ? boundedBasename(value) : "unavailable"; }
function settingsEffectText(settings: Record<string, unknown>): string {
  const after = typeof settings.effectiveAfter === "object" && settings.effectiveAfter !== null && !Array.isArray(settings.effectiveAfter) ? settings.effectiveAfter as Record<string, unknown> : undefined;
  const effectiveAfter = after?.present === true
    ? `present; value=${settingsValueText(after.value)}; scope=${safe(after.scope ?? "unknown", 40)}; source=${settingsSourceText(after.source)}`
    : after?.present === false ? "absent" : "unavailable";
  const requested = Object.hasOwn(settings, "requested") ? settings.requested : settings.setting ?? "unknown";
  const requestedText = requested === null ? "declaration-removal" : settingsValueText(requested);
  return `requested=${requestedText}; requested-authority-achieved=${safe(settings.effective ?? "unknown", 40)}; effective-after=${effectiveAfter}; declaration-only=${safe(settings.declarationOnly ?? "unknown", 20)}`;
}
...
function renderPluginPreview(preview: PluginMutationPreview, operation: Extract<PluginInventoryOperation, { kind: "install" | "enable" | "disable" | "update" | "uninstall" }>): string {
  ...
  return ["Plugin lifecycle preview", `Operation ID: ${safe(preview.operationId, 128)}`, `Operation/action/target: plugin ${preview.action}; ${safe(preview.pluginId)}`, `Target scope/record selector: ${preview.scope}; ${safe(operation.flags.selector ?? "new record", 1024)}`, `Selected marketplace selector: ${safe(marketplaceSelection, 1024)}`, `Immutable catalog: ${preview.catalog === undefined ? "unchanged" : `${safe(preview.catalog.marketplaceName, 128)}; snapshot=${safe(preview.catalog.snapshotId, 128)}; digest=${digestText(preview.catalog.catalogDigest)}`}`, `Source authority: ${sourceAnchorText(preview.requestedSource ?? preview.source)}`, `Source kinds: requested=${preview.requestedSource?.kind ?? "unchanged"}; durable=${preview.source?.kind ?? "unchanged"}`, `Immutable revision: ${safe(preview.immutableRevision ?? "unchanged")}`, `Digests: artifact=${digestText(preview.artifactDigest)}; tree=${digestText(preview.treeDigest)}; root=${digestText(preview.rootDigest)}; executable=${digestText(preview.executableDigest)}`, `Executable content: ${preview.executableComponents.length > 0 ? preview.executableComponents.length : preview.executableDigest === undefined ? "unavailable" : `unchanged admitted executable content; digest=${digestText(preview.executableDigest)}`}`, ...preview.executableComponents.map((item) => `- ${componentText(item)}`), `Dependencies: admitted=${preview.dependencies.selected.admitted}; blocking=${preview.dependencies.blocking}; reasons=${safe(dependencyReasons.join(", ") || "none")}; graph=${preview.dependencies.graph.length}`, ...completeLines(dependencyReasons), ...graph, ...decisions, `Trust: ${preview.trust === undefined ? "existing authority" : `required for ${safe(preview.trust.target)}; artifact=${digestText(preview.trust.artifactDigest)}; executable=${digestText(preview.trust.executableDigest)}`}`, `Default/enablement: ${preview.enablement === undefined ? "preserved or unchanged" : `enabled=${preview.enablement.enabled}; source=${preview.enablement.source}`}`, `Settings/declaration: ${preview.settingsEffect === undefined ? "unchanged" : `${preview.settingsEffect.setting}; ${settingsEffectText(preview.settingsEffect as unknown as Record<string, unknown>)}`}`, `Destructive choices: remove-declaration=${preview.removeDeclaration}; remove-data=${preview.removeData}`, `Participants: ${preview.participants.length}`, ...preview.participants.map((item) => `- ${item.kind}:${item.effect}:${item.targetClass}`), "Expected state changes:", ...completeLines(preview.consequences)].join("\n");
}
...
function recoverySummaryLines(summary: unknown, completed: number): string[] {
  ...
  const snapshot = typeof value.snapshot === "object" && value.snapshot !== null ? value.snapshot as Record<string, unknown> : undefined; const catalog = typeof value.catalog === "object" && value.catalog !== null ? value.catalog as Record<string, unknown> : undefined;
  ...
  const dependencies = typeof value.dependencies === "object" && value.dependencies !== null ? value.dependencies as Record<string, unknown> : undefined; const selected = typeof dependencies?.selected === "object" && dependencies.selected !== null ? dependencies.selected as Record<string, unknown> : undefined; const reasons = Array.isArray(selected?.reasons) ? selected.reasons.filter((item): item is string => typeof item === "string") : [];
  const trust = typeof value.trust === "object" && value.trust !== null ? value.trust as Record<string, unknown> : undefined; const snapshotTrust = typeof snapshot?.trust === "object" && snapshot.trust !== null ? snapshot.trust as Record<string, unknown> : undefined; const components = Array.isArray(value.executableComponents) ? value.executableComponents.filter((item): item is string => typeof item === "string") : [];
  const plugin = typeof value.pluginId === "string"; const sourceAuthority = value.requestedSource ?? value.source ?? registration?.source; const immutable = plugin
    ? `revision=${safe(value.immutableRevision ?? "unavailable", 160)}; artifact=${digestText(value.artifactDigest)}; tree=${digestText(value.treeDigest)}; root=${digestText(value.rootDigest)}; executable=${digestText(value.executableDigest)}`
    : `snapshot=${safe(snapshot?.snapshotId ?? "unavailable", 160)}; catalog=${digestText(snapshot?.catalogDigest)}; trust=${digestText(snapshotTrust?.targetDigest)}`;
  const executable = plugin ? components.length > 0 ? `${components.length}; ${components.map(componentText).join(", ")}` : value.executableDigest === undefined ? "unavailable" : `unchanged admitted executable content; digest=${digestText(value.executableDigest)}` : "marketplace catalog is inert; installed executable membership is unchanged";
  return [`Producer target/scope/action: ${safe(target, 160)}; ${safe(scope, 80)}; ${safe(value.action ?? "unknown", 80)}`, `Source authority: ${sourceAnchorText(sourceAuthority)}`, `Immutable authority: ${immutable}`, `Executable content posture: ${executable}`, `Dependencies/catalog posture: ${plugin ? `admitted=${safe(selected?.admitted ?? "unavailable", 20)}; blocking=${safe(dependencies?.blocking ?? "unavailable", 20)}; reasons=${safe(reasons.join(", ") || "none", 1024)}; graph=${Array.isArray(dependencies?.graph) ? dependencies.graph.length : "unavailable"}` : `declarations=${Array.isArray(catalog?.plugins) ? catalog.plugins.length : "unavailable"}; omitted=${safe(catalog?.omittedEntries ?? "unavailable", 20)}`}`, `Trust posture: ${plugin ? trust === undefined ? "existing trust authority unavailable" : `target=${safe(trust.target ?? "unavailable", 160)}; artifact=${digestText(trust.artifactDigest)}; root=${digestText(trust.rootDigest)}; executable=${digestText(trust.executableDigest)}` : `catalog trust=${digestText(snapshotTrust?.targetDigest)}`}`, `Settings/declaration effect: ${settings === undefined ? "unchanged or unavailable" : settingsEffectText(settings)}`, `Record/generation/data/destructive effects: record=${safe(value.mutableRecordKey ?? "unchanged", 160)}; generation=${safe(value.generationId ?? "unchanged", 120)}; remove-declaration=${safe(value.removeDeclaration ?? "false", 20)}; remove-data=${safe(value.removeData ?? "false", 20)}; acknowledgement=${safe(value.acknowledgement ?? "none", 80)}`, `Participant effects: total=${participants.length}; ${participantText.join(", ") || "none"}`, `Producer consequences: total=${consequences.length}`, ...completeLines(consequences)];
}
...
function discardStatus(kind: "Marketplace" | "Plugin", confirmation: Confirmation, operationId: string, discarded: StoreResult<void>): string {
  if (!discarded.ok) return `${kind} lifecycle ${confirmation === "cancelled" ? "was cancelled" : "confirmation was unavailable"}, but preview cleanup could not be confirmed. Operation ID: ${safe(operationId, 128)}. Preserve any lifecycle staging and retry deliberately; no lifecycle transaction execution or durable desired-state commit began.`;
  return confirmation === "cancelled" ? `${kind} lifecycle operation was cancelled by the user. Staging was discarded; no lifecycle transaction execution or durable desired-state commit began.` : `${kind} lifecycle confirmation is unavailable on noninteractive input. Re-run with --yes after reviewing the preview. Staging was discarded; no lifecycle transaction execution or durable desired-state commit began.`;
}
async function refuseUnsafePreview(kind: "Marketplace" | "Plugin", operationId: string, discard: (operationId: string) => Promise<StoreResult<void>>, output: PluginInventoryCliOutput): Promise<number> {
  const discarded = await discard(operationId); const status = discarded.ok ? "Staging was discarded; no lifecycle transaction execution or durable desired-state commit began." : "Staging cleanup could not be confirmed; preserve it. No lifecycle transaction execution or durable desired-state commit began.";
...
function presentAcquisitionPlanningNotice(operation: Exclude<PluginInventoryOperation, { kind: "list" | "details" }>, output: PluginInventoryCliOutput): boolean {
  if (operation.kind !== "marketplace-add" && operation.kind !== "marketplace-refresh" && operation.kind !== "install" && operation.kind !== "update") return true;
  try { output.log("Lifecycle planning may acquire, validate, and stage an immutable candidate for review now. Final confirmation is required before lifecycle transaction execution or durable desired-state commit; cancelling attempts to discard candidate staging."); return true; }
  catch { try { output.error("Lifecycle planning notice could not be presented; planning did not start."); } catch {} return false; }
}
...
async function runLifecycle(operation: Exclude<PluginInventoryOperation, { kind: "list" | "details" }>, services: PluginLifecycleCliServices, output: PluginInventoryCliOutput, options: PluginInventoryCliOptions): Promise<number> {
  ...
  if (operation.kind.startsWith("marketplace-")) {
    if (!presentAcquisitionPlanningNotice(operation, output)) return 1;
    ...
    try { output.log(text); } catch { const discarded = await services.marketplaces.discardPreview(planned.value.operationId); try { output.error(discarded.ok ? `Preview output failed. Operation ID: ${safe(planned.value.operationId, 128)}. Staging was discarded; no lifecycle transaction execution or durable desired-state commit began.` : `Preview output failed and staging cleanup is uncertain. Operation ID: ${safe(planned.value.operationId, 128)}. Preserve any lifecycle staging; no lifecycle transaction execution or durable desired-state commit began.`); } catch {} return 1; }
  ...
  try { output.log(text); } catch { const discarded = await services.plugins.discardPreview(planned.value.operationId); try { output.error(discarded.ok ? `Preview output failed. Operation ID: ${safe(planned.value.operationId, 128)}. Staging was discarded; no lifecycle transaction execution or durable desired-state commit began.` : `Preview output failed and staging cleanup is uncertain. Operation ID: ${safe(planned.value.operationId, 128)}. Preserve any lifecycle staging; no lifecycle transaction execution or durable desired-state commit began.`); } catch {} return 1; }
...
type LoadedMarketplaceRegistration = ReturnType<typeof loadPluginMarketplaceState>["registrations"][number];
function localPathForComparison(value: string): string {
  let normalized = path.normalize(value); const root = path.parse(normalized).root;
  while (normalized.length > root.length && normalized.endsWith(path.sep)) normalized = normalized.slice(0, -path.sep.length);
  return process.platform === "win32" ? normalized.toLowerCase() : normalized;
}
function sameMarketplaceSourceAuthority(left: MarketplaceRegistrationSource, right: MarketplaceRegistrationSource): boolean {
  const leftLocal = left.kind === "local-directory" || left.kind === "local-catalog-file";
  const rightLocal = right.kind === "local-directory" || right.kind === "local-catalog-file";
  if (leftLocal || rightLocal) return leftLocal && rightLocal && left.kind === right.kind && localPathForComparison(left.path) === localPathForComparison(right.path);
  return same(left, right);
}
function lifecycleSource(registration: LoadedMarketplaceRegistration, sourceAnchor: string): MarketplaceRegistrationSource | undefined {
  const value = registration.source; const kind = value["kind"];
  if ((kind === "directory" || kind === "file") && typeof value["localPath"] === "string") {
    let localPath = value["localPath"];
    if ((registration.provenance.scope === "project" || registration.provenance.scope === "local") && registration.validity === "valid" && !path.isAbsolute(localPath)) localPath = path.resolve(sourceAnchor, localPath.replaceAll("/", path.sep).replaceAll("\\", path.sep));
    return kind === "directory" ? { kind: "local-directory", path: localPath } : { kind: "local-catalog-file", path: localPath };
  }
...
async function createPluginLifecyclePort(project: LoadedProject, options: PluginInventoryCliOptions, freshCommandAuthority?: LoadedProject): Promise<StoreResult<PluginLifecyclePort>> {
  ...
  const marketplaceObservations = (): readonly MarketplaceObservation[] => {
    ...
    return currentState.registrations.flatMap((registration) => {
      const sourceValue = lifecycleSource(registration, family); if (sourceValue === undefined) return [];
      const canMatchOwned = registration.provenance.scope !== "project" && registration.provenance.scope !== "local" || registration.validity === "valid";
      const ownedMatches = canMatchOwned ? applicableRegistrations.filter((record) => record.name === registration.name && record.scope === registration.provenance.scope && sameMarketplaceSourceAuthority(record.source, sourceValue)) : [];
