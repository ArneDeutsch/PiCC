...
const SGR_RESET_RE = /\u001b\[0m/gu;
const INLINE_CONTROL_RE = /[\u0000-\u001f\u007f-\u009f]/u;
...
/**
 * Fit an already-sanitized plain fragment with Pi TUI's column semantics.
 * Pi TUI brackets a truncation ellipsis with SGR resets even for plain input;
 * remove only those known generated bytes so callers can decorate the complete
 * fitted fragment afterward.
 */
export function fitPlainFragment(text: string, width: number): string {
  try {
    if (INLINE_CONTROL_RE.test(text) || !Number.isFinite(width) || width <= 0) return "";
    const fitted = truncateToWidth(text, Math.floor(width), "…").replace(SGR_RESET_RE, "");
    return INLINE_CONTROL_RE.test(fitted) ? "" : fitted;
  } catch {
    return "";
  }
}
