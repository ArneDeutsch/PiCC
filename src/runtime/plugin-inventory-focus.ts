...
function confirmationProjection(preview: MarketplaceMutationPreview | PluginMutationPreview | RecoveryPreview, action: PluginInventoryActionName, exact: readonly PluginLifecycleExactTarget[], recoveryAction?: "complete" | "rollback"): PluginInventoryConfirmationProjection {
  ...
  if ("pluginId" in preview) {
    ...
    return Object.freeze({ operationId: scalar(preview.operationId), action, target: scalar(preview.pluginId), authority, sourceAuthority: scalar(safeSourceAuthority(preview.requestedSource ?? preview.source)),
      ...
      executable: preview.executableComponents.length > 0 ? list(preview.executableComponents, (value) => scalar(value)) : preview.executableDigest === undefined ? Object.freeze([]) : Object.freeze([`unchanged admitted executable content · digest ${scalar(immutableEvidence(preview.executableDigest))}`]), destructive: Object.freeze([`remove declaration ${preview.removeDeclaration ? "yes" : "no"}`, `remove data ${preview.removeData ? "yes" : "no"}`]),
      ...
      sessionBehavior: Object.freeze(["candidate acquisition or staging may have occurred during planning; lifecycle transaction execution and durable desired-state commit require confirmation", "loaded generation, components, outcomes, and badges stay fixed until successful reload or a new session"]), recovery: Object.freeze([]), omissions });
  }
  ...
  return Object.freeze({ operationId: scalar(preview.operationId), action, target: scalar(summary.pluginId ?? registration?.name ?? preview.operationId), authority,
    ...
    resolution: Object.freeze([`producer ${scalar(preview.producerSchema)} v${scalar(preview.producerVersion)}`, `confirmation ${scalar(preview.confirmationDigest)}`, `plan ${scalar(preview.planDigest)}`, ...(plugin ? [`revision ${scalar(immutableEvidence(summary.immutableRevision))}`, `artifact ${scalar(immutableEvidence(summary.artifactDigest))}`, `tree ${scalar(immutableEvidence(summary.treeDigest))}`, `root ${scalar(immutableEvidence(summary.rootDigest))}`, `executable ${scalar(immutableEvidence(summary.executableDigest))}`, `generation ${scalar(summary.generationId ?? "unchanged")}`] : [`snapshot ${scalar((summary.snapshot as Record<string, unknown> | undefined)?.snapshotId)}`, `catalog digest ${scalar(immutableEvidence((summary.snapshot as Record<string, unknown> | undefined)?.catalogDigest))}`]), `completed ${scalar(preview.completed)} · remaining ${scalar(preview.remaining)} · rolled back ${scalar(preview.rolledBack)}`]),
    trust: Object.freeze([plugin ? (() => { const trust = summary.trust as Record<string, unknown> | undefined; return `target ${scalar(trust?.target ?? "existing trust authority")} · artifact ${scalar(immutableEvidence(trust?.artifactDigest))} · root ${scalar(immutableEvidence(trust?.rootDigest))} · executable ${scalar(immutableEvidence(trust?.executableDigest))}`; })() : `marketplace trust ${scalar(immutableEvidence(((summary.snapshot as Record<string, unknown> | undefined)?.trust as Record<string, unknown> | undefined)?.targetDigest))}`]),
    ...
    settings: settingsProjection(typeof summary.settingsEffect === "object" && summary.settingsEffect !== null ? summary.settingsEffect as PluginSettingsEffectSummary : undefined), executable: plugin ? Array.isArray(summary.executableComponents) && summary.executableComponents.length > 0 ? list(summary.executableComponents, (value) => scalar(value)) : summary.executableDigest === undefined ? Object.freeze([]) : Object.freeze([`unchanged admitted executable content · digest ${scalar(immutableEvidence(summary.executableDigest))}`]) : Object.freeze(["marketplace catalog is inert; installed executable membership is unchanged"]),
...
export class PluginInventoryFocusController implements PluginInventoryFocusComponent {
  ...
  render(width: number): string[] {
    ...
    if ((workflow?.phase === "preview" || workflow?.phase === "confirmation") && columns < 8) {
      ...
      const lines: string[] = []; pushWrapped("Active workflow; candidate staging may exist; lifecycle transaction execution and durable desired-state commit require confirmation.", columns, lines); pushWrapped("Resize", columns, lines); pushWrapped("Esc", columns, lines);
      return clampLines(lines, columns);
    }
    try {
  ...
  private async planForm(): Promise<void> {
    ...
    try {
      ...
      if (epoch === this.workflowEpoch) {
        ...
        this.model.failWorkflow("Lifecycle planning failed closed. Check lifecycle service availability and retry planning. No lifecycle execution or durable desired-state commit began; candidate acquisition or staging may have occurred and cleanup could not be confirmed.", action, undefined, target);
      }
    }
  ...
  private planFailure(result: { readonly code: string; readonly message: string }, form: NonNullable<PluginInventoryFocusController["form"]>, _privateValues: readonly string[] = []): void {
    ...
    this.clearPrivateInput(); this.model.failWorkflow(`${presentation.cause}. ${presentation.guidance} No lifecycle execution or durable desired-state commit began; candidate acquisition or staging may have occurred and cleanup could not be confirmed.`, form.action, undefined, form.target);
  }
  ...
  private async failAndDiscard(state: Extract<NonNullable<ReturnType<PluginInventoryModel["workflow"]>>, { phase: "preview" | "confirmation" }>, message: string): Promise<void> {
    ...
    if (epoch !== this.workflowEpoch || this.disposed) return;
    this.clearPrivateInput(); this.model.failWorkflow(discarded.ok ? `${message} Staging was discarded; no lifecycle transaction execution or durable desired-state commit began.` : `${message} Staging cleanup is uncertain; candidate staging may remain, but no lifecycle transaction execution or durable desired-state commit began.`, state.action, state.operationId, state.target); this.repaint();
  }
