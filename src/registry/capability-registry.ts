...
const SUPPORTED_HOOK_EVENT_NOTES: Record<(typeof SUPPORTED_HOOK_EVENTS)[number], string> = {
  ...
  SessionStart: "fires at session start and matches startup|resume|clear|compact|fork; Pi 0.80.6 startup and reload reasons map to startup, new maps to clear, and resume/fork are preserved, while compact is emitted synthetically after compaction commits. For compact-source main and child events, stdout/additionalContext is restored, ordinary exit-2/decision:block output is non-blocking, and universal continue:false terminates the committed-summary session. PARTIAL because the complete cross-scope contract is not verified",
