...
const SKILL_FRONTMATTER_FULL_NOTES: Record<string, string> = {
  ...
  context: "context: fork runs the skill as a fresh-context subagent; a fork that fails on a terminal error is a LOUD failure naming the cause with partial output preserved (parity with the Agent tool, 2.1.199); F14 threads the Esc signal to a model-invoked context: fork (the Skill or SlashCommand tool) so it reports aborted, but a typed top-level /forked-skill expansion is NOT Esc-cancellable — a PiCC/Pi harness limitation (no abort signal at the input-hook stage), not Claude Code scoping Esc; fork dispatches are non-resumable (§4.1)",
