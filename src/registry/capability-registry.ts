...
const TOOL_ENTRIES: CapabilityEntry[] = [
  ...
  cap("tool", "tool.TaskStop", "partial", "stops a background subagent — every model-visible stop result uses PiCC-defined wording (not verified as exact Claude wording) to correlate Task(task-N), the requested/displayed Agent(<type>), and stable agent-<id>; unlike TaskStop/settlement, a SendMessage resume acknowledgment uses the clean resolved registry name; stopping is cooperative via session abort; a stopped run reports a stopped status and its late result is discarded — PiCC-defined behavior because Claude Code leaves TaskStop's post-stop result semantics undocumented (§4.3, §4.8)"),
  cap("tool", "tool.SendMessage", "partial", "coordinator's channel back into its subagents (Claude 2.1.x): supports resuming a completed or failed-with-partial subagent by agent id, re-dispatched in the background with full prior context, or steering a running background one; a resumed failed/completed agent flips back to a running status (2.1.205). PiCC's model-visible resume acknowledgment wording (not verified as exact Claude wording) correlates the new Task(task-N), clean resolved-registry Agent(<type>), and stable agent-<id>, unlike TaskStop/settlement's requested/displayed type; GAPS: no cross-restart resume (the dispatch registry is process-lifetime), steering reaches only background dispatches (a foreground Agent call blocks the parent's turn), an idle coordinator learns of a settlement only at its next turn, fork/agentOverride dispatches are non-resumable; no subagent-to-subagent / agent-teams messaging (§4.3, §7)"),
...
const FEATURE_ENTRIES: CapabilityEntry[] = [
  ...
  cap("feature", "feature.background-agents", "partial", "run_in_background / background:true dispatches with the TaskOutput/TaskStop lifecycle; a failed background task is reported as failed with its cause (never an empty success, t01) and settlement (success or failure) is pushed to the coordinator at its next turn without polling (t05); PiCC-defined model-visible wording (not verified as exact Claude wording) correlates Task(task-N), Agent(<type>), and stable agent-<id>: TaskStop results and pushed settlement notices use the requested/displayed type, while a SendMessage resume acknowledgment uses the clean resolved registry name and a new task id with the same stable agent id; a background task is observable while a TaskOutput call is awaiting it — the same live rolling-tail + activity render and settle badge a foreground dispatch shows, with the start message and awaiting/live, poll, and settled TaskOutput views naming the same identity components (F04); GAPS: PiCC defaults foreground whereas Claude 2.1.198 runs subagents background-by-default, an idle coordinator is not re-invoked (it learns of settlement only when the conversation continues), settlement notices are bounded/excerpted, no always-on Agent View dashboard (Claude Code's `claude agents` shell command, 2.1.139+) — background progress is observable only while a TaskOutput call is awaiting it, never an unattended panel, no remote/cloud agents, stop is cooperative (§4.3)"),
