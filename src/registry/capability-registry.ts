...
const SUPPORTED_HOOK_EVENT_NOTES: Record<(typeof SUPPORTED_HOOK_EVENTS)[number], string> = {
  ...
  SessionStart: "fires at session start and matches startup|resume|clear|compact|fork; Pi 0.80.6 startup and reload reasons map to startup, new maps to clear, and resume/fork are preserved, while compact is emitted synthetically after compaction commits. PARTIAL because compact-source payload/order/control/output semantics are implemented for main and child sessions but the complete cross-scope contract is not verified",
  ...
  PreCompact: "fires before compaction with manual|auto trigger and can block; PARTIAL because complete payload/order/control/output semantics are not verified across both main and child sessions; a blocked proactive checkpoint fails closed instead of continuing uncompacted as Claude Code does",
  PostCompact: "fires only after compaction commits; PARTIAL because complete payload/order/control/output semantics are not verified across both scopes, and restoration order differs: main fires after SessionStart context but before skill restoration, while children fire after SessionStart context and skill restoration",
...
const HOOK_EVENT_ENTRIES: CapabilityEntry[] = [
  ...SUPPORTED_HOOK_EVENTS.map((ev) =>
    cap(
      "hook-event",
      `hook.event.${ev}`,
      ev === "PreCompact" || ev === "PostCompact" || ev === "SessionStart" ? "partial" : "full",
