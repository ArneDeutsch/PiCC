...
const TOOL_ENTRIES: CapabilityEntry[] = [
  ...
  cap("tool", "tool.Task", "partial", "alias of the Agent subagent-dispatch tool — same loud-failure/agent-id/progress semantics and the same background-by-default routing (Claude 2.1.198) with the next-turn-settlement timing residual; subagent_type:\"fork\" inherits the parent conversation — see tool.Agent.fork (§4.3)"),
  ...
  cap("tool", "tool.SendMessage", "partial", "resumes a completed/failed-with-partial subagent by agent id in the background with prior context, or steers a running background one (Claude 2.1.x); resumed failed/completed agents return to running (2.1.205). GAPS: PiCC allows resume after TaskStop, while the Claude Code 2.1.x reference refuses stopped-agent resume; no cross-restart resume; steering is background-only; idle-parent settlement delivery is next-turn; fork dispatches are non-resumable (their inherited parent-conversation context can't be safely re-derived — see tool.Agent.fork), and agentOverride and subagent-to-subagent/agent-teams messaging are unsupported. The resume acknowledgment includes the new task id, resolved registry name, and stable agent id in PiCC-defined model-visible wording, not verified as exact Claude wording (§4.3, §7)"),
