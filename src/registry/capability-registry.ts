...
const TOOL_ENTRIES: CapabilityEntry[] = [
  ...
  cap("tool", "tool.NotebookEdit", "degraded-noop", "callable no-op stub — notebook editing not implemented; the notice directs editing the .ipynb as JSON via Read/Edit"),
...
const SUPPORTED_HOOK_EVENT_NOTES: Record<(typeof SUPPORTED_HOOK_EVENTS)[number], string> = {
  ...
  UserPromptSubmit: "fires on user prompt; stdout injected as context",
...
const FRONTMATTER_ENTRIES: CapabilityEntry[] = [
  ...
  cap("frontmatter", "agent.frontmatter.color", "degraded-noop", "parsed only — cosmetic; Pi's TUI does not render agent colors"),
