...
interface PluginInventoryComponentBase {
  readonly declaration?: "path" | "paths" | "object" | "default-layout";
...
export type PluginInventoryComponent = PluginInventoryComponentBase & (
  ...
  | { readonly origin: "final-runtime"; readonly kind: Exclude<ComponentKind, "hooks">; readonly countSemantics: "finalized-registrations"; readonly posture: "final-loaded" }
...
export interface PluginInventoryCapabilityEvidence { readonly capabilityId: string; readonly qualifiedIdentity: string; readonly component?: string; readonly observation: string }
...
export interface PluginInventoryItem {
  ...
  readonly dependencies: readonly { readonly targetIdentity: string; readonly version?: string; readonly versionStatus?: string; readonly posture: string; readonly crossMarketplace: string; readonly provenance: PluginInventoryProvenance }[];
...
function componentCapability(kind: ComponentKind): string { return ({ skills: "feature.plugins-skills", commands: "feature.plugins-commands", agents: "feature.plugins-agents", hooks: "feature.plugins-hooks", mcpServers: "feature.mcp-plugin-servers", lspServers: "feature.plugins-other-components" })[kind]; }
function risk(kind: ComponentKind): PluginInventoryComponent["executionRisk"] { return kind === "skills" ? "context" : kind === "mcpServers" ? "network-or-process" : kind === "lspServers" ? "unsupported-runtime" : "code"; }
function component(kind: ComponentKind, origin: PluginInventoryComponent["origin"], count: number, declaration?: PluginInventoryComponent["declaration"], itemProvenance?: PluginInventoryProvenance): PluginInventoryComponent {
  ...
  if (origin === "selected-manifest") return Object.freeze({ ...base, origin, countSemantics: "selected-manifest-declarations" });
  if (origin === "catalog") return Object.freeze({ ...base, origin, countSemantics: "catalog-declarations", posture: "declared-not-effective" });
  if (kind === "hooks") return Object.freeze({ ...base, kind, origin, countSemantics: "retained-executable-registrations", posture: "final-loaded" });
  return Object.freeze({ ...base, kind, origin, countSemantics: "finalized-registrations", posture: "final-loaded" });
}
function metadata(value: SafePluginManifestProjection): SafePluginManifestProjection { return Object.freeze({ ...value, keywords: Object.freeze(value.keywords.slice(0, 32).map((item) => boundedText(item))), components: Object.freeze(value.components.slice(0, 64).map((item) => Object.freeze({ ...item }))), omissions: Object.freeze({ keywords: value.omissions?.keywords ?? Math.max(0, value.keywords.length - 32), components: value.omissions?.components ?? Math.max(0, value.components.length - 64), diagnostics: value.omissions?.diagnostics ?? 0 }) }); }
...
export function buildPluginInventorySnapshot(options: BuildPluginInventorySnapshotOptions): PluginInventorySnapshot {
  ...
  for (const qualifiedIdentityRaw of idList) {
    ...
    if (selectedPlugin !== undefined) for (const declaration of selectedPlugin.manifestProjection.components) components.push(component(declaration.field, "selected-manifest", declaration.count, declaration.declaration));
    ...
    if (finalCounts !== undefined) for (const kind of ["skills", "commands", "agents", "hooks"] as const) if (finalCounts[kind] > 0) components.push(component(kind, "final-runtime", finalCounts[kind], "default-layout"));
    for (const entry of catalogEntries) for (const [kind, componentDeclarations] of Object.entries(entry.components) as Array<[ComponentKind, NonNullable<typeof entry.components[keyof typeof entry.components]>]>) for (const declaration of componentDeclarations) {
      const base = component(kind, "catalog", declaration.kind === "object-shape" ? declaration.shape.keys.length : 1, declaration.kind === "path" ? "path" : "object", provenance(declaration.provenance, options, entry.provenance));
    ...
    const dependencyValues = catalogEntries.flatMap((entry) => entry.dependencies.map((value) => ({ value, context: entry.provenance })));
    const dependencies = cap(dependencyValues, LIMIT.dependencies, omissions, "snapshot.dependencies").map(({ value, context }) => Object.freeze({ targetIdentity: safeToken(value.targetIdentity, "unknown@unknown"), ...(value.version === undefined ? {} : { version: boundedText(value.version) }), ...(value.versionStatus === undefined ? {} : { versionStatus: value.versionStatus }), posture: value.posture, crossMarketplace: value.crossMarketplace, provenance: provenance(value.provenance, options, context) }));
    ...
    if (selectedPlugin !== undefined) for (const [key, count] of Object.entries(selectedPlugin.manifestProjection.omissions ?? {})) addOmission(omissions, `snapshot.metadata.${key}`, count);
