...
import { isDocumentedMarketplaceName, normalizeMarketplaceCatalogDocument, type MarketplaceCatalogDeclarationSummary } from "../util/plugin-marketplace-descriptor.js";
import {
...
import { observePersistedTransactionsSync, prepareTransaction, readTransactionJournal, readTransactionReceipt, type PreparedTransaction, type TransactionParticipant } from "./transaction.js";
...
export function createMarketplaceAcquisitionAdapter(options: MarketplaceAcquisitionAdapterOptions): MarketplaceServiceDependencies["acquire"] {
  return async (name, source, signal) => {
    if (source.kind === "https-catalog") {
      ...
      const target: MarketplaceSnapshotTrustTarget = { authorityKind: "catalog-only", marketplaceName: name, snapshotId: acquired.value.snapshotId, source: acquired.value.source as Extract<MarketplaceRegistrationSource, { kind: "https-catalog" }>, catalogDigest: acquired.value.catalogDigest, provenance: { adapter: "public-https-catalog", canonicalUrl: (acquired.value.provenance.reviewed as { canonicalUrl: string }).canonicalUrl } };
    ...
    if (source.kind === "local-directory" || source.kind === "local-catalog-file") {
      ...
      const tree = acquired.value.materialized; const catalog = catalogPlugins(privateEvidence.catalog, source.kind);
      if (!catalog.ok || catalog.value.name !== name) { await discardMaterializedPluginTree(tree); return !catalog.ok ? catalog : fail("catalog-name-mismatch", "Resolved catalog identity does not match the selected marketplace name"); }
      ...
      const target: MarketplaceSnapshotTrustTarget = { authorityKind: "materialized", marketplaceName: name, snapshotId: acquired.value.snapshotId, source: acquired.value.source as Exclude<MarketplaceRegistrationSource, { kind: "https-catalog" }>, catalogDigest: acquired.value.catalogDigest, artifactDigest: tree.treeDigest, treeDigest: tree.treeDigest, rootDigest: tree.treeDigest,
    ...
    const catalog = catalogPlugins(catalogBytes, source.kind); if (!catalog.ok || catalog.value.name !== name) { await discardMaterializedPluginTree(acquired.value.materialized); return !catalog.ok ? catalog : fail("catalog-name-mismatch", "Resolved catalog identity does not match the selected marketplace name"); }
    ...
    const target: MarketplaceSnapshotTrustTarget = { authorityKind: "materialized", marketplaceName: name, snapshotId: acquired.value.snapshotId, source, catalogDigest: acquired.value.catalogDigest, artifactDigest: acquired.value.materialized.treeDigest, treeDigest: acquired.value.materialized.treeDigest, rootDigest: acquired.value.materialized.treeDigest,
...
async function cleanupParticipants(store: OwnedStateStore, participants: readonly TransactionParticipant[]): Promise<StoreResult<void>> {
  try {
    const candidates = participants.flatMap((participant) => [participant.stagedPath, participant.rollback.kind === "restore-backup" ? participant.rollback.path : undefined]).filter((candidate): candidate is string => candidate !== undefined && isContainedPath(store.stagingRoot, candidate));
...
export class PluginMarketplaceService {
  ...
  async #plan(operationId: string, action: MarketplaceMutationAction, row: MarketplaceObservation, snapshot: OwnedMarketplaceSnapshotRecord, catalog: MarketplaceCatalogDeclarationSummary, value: PluginMarketplaceRegistrationSource | undefined, options: MarketplaceMutationOptions, consequences: readonly string[], captured: CapturedObservations, token: symbol): Promise<StoreResult<MarketplaceMutationPreview>> {
    ...
    const recordParticipants: TransactionParticipant[] = []; const summaries: MarketplaceParticipantSummary[] = []; const allParticipants = (): TransactionParticipant[] => [settings.value.transaction.participants[0]!, ...recordParticipants];
    ...
    try {
      const settingsParticipant = settings.value.transaction.participants[0]!; const settingsEvidence = canonicalJsonBytes(settingsParticipant.producerEvidence); if (!settingsEvidence.ok) return abort(settingsEvidence);
