...
2. **Stay inside the writable surface** named in the task spec. Everything else is read-only. No workarounds, no mocking-away of problems, no scope creep beyond the spec.
3. **Implement the goal**, deciding the "Left open" items as you go. Prefer the idioms of the surrounding code.
4. **Verify.** Run `npm run typecheck` and the tests the task requires (`npm test`, or the narrower command the spec names). They must be green, or show no new failures versus the baseline the coordinator gave you. Report the exact result summary.
5. **Keep the execution log** at the path the task spec names (part of your writable surface): brief bullets — key decisions (especially on "Left open" items), deviations from the spec, friction, and anything surprising you found in the existing code.
