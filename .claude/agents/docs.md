...
**Investigate** — asked what documentation a planned change implies. Lead with one concise disposition: either a justified **no durable documentation change** result after confirming existing claims remain true, or the smallest sufficient set under the guide's complete named *Proportional scope* test. State whether anti-drift investigation finds that support truth in the capability registry changed (⇒ mandatory registry repair and regeneration); proportional scope decides only whether additional prose is warranted. Apply the guide's distinct comment rule: propose a source comment only for a local non-obvious why, and preserve existing load-bearing rationale and comment-shaped directives.
...
- Completeness: user-visible or architectural changes receive new durable prose only when the guide's *Proportional scope* test warrants it; regardless of additions, repair any existing claim the realized behavior invalidates.
...
- Proportionality: after mandatory truth repairs, apply *Proportional scope* to the whole proposed or realized set. Reject or remove aggregate excess — including an entire documentation task — even if every statement is true and correctly placed. A no-change result passes only after the anti-drift check; review never presumes additions.
- Genre boundaries: capability support truth and its mechanical regeneration remain mandatory when affected; optional prose is separate. Comments added or changed must explain a local non-obvious *why*, while retained load-bearing rationale and comment-shaped directives are not removable as volume reduction. Apply the guide's comment checklist.
...
- If no documentation surface is touched or implied and existing claims remain true, say PASS with the concise no-change reason; never fabricate additions or findings.
