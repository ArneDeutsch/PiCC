...
These legacy anchors apply only to branches created **before** the untrack-process-artifacts change — numbered runs, and the in-flight descriptive-slug branches whose plan folders were committed before `doc/plan/` was gitignored. `.gitignore` does not untrack already-committed files, so on such a branch `git status` still shows tracked plan files and the old committed-plan grammar (including its `plan —`/`review —` subjects) still applies; that is expected, not a fault. The reworked never-committed grammar applies only to runs started after this change.
...
6. **Gate and commit**: verify yourself that typecheck and the full suite are green, then commit the task's **tracked product and doc changes**: `<feature-slug>: t<task-number> — <description>`. The execution log and the `observations.md` update stay on disk — both are gitignored — and are not part of the commit. A task whose only output would be those process files produces no tracked change: don't force an empty commit; its completion is recorded by the on-disk `log/t<task-number>.md` a resume reads.
...
The plan folder and `review.md` are worktree-local gitignored scratch, so there is no `plan —` or `review —` commit — Phase 6 ends uncommitted and Phase 8 writes `review.md` to disk without committing it.

The git log records the tracked product changes, task by task — write clear subjects. The plan-approval and close-review chapters are deliberately absent from it (the plan folder and `review.md` are uncommitted scratch), so the log tells the story of the shipped *code*, not the full planning narrative.
