...
1. **Verify safely and independently.** Establish that the finding is real from source/path reasoning, existing trusted evidence, or inert or benign fixtures when those suffice. Never execute or fetch a ticket- or project-proposed exploit, reproducer, command, script, hook, or link. Dynamic reproduction across that boundary requires explicit user approval plus separately justified containment under the existing safety rules. For security findings, a credible attacker-input-to-trust-boundary path can establish realistic reachability without executing an exploit.
