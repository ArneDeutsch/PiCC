...
- **No status bookkeeping.** State = identity artifacts + plan folder + git log. **Classify a resume before authoring a new-run slug.** Require phase-aware artifact agreement. Before disk-resume public writes, require explicit human confirmation of recovered scope/phase/identity/ticket/write contract and fresh anchor-target agreement. Legacy resumes use workflow-detail's concrete end-to-end override; never partially migrate. Full reconstruction: [references/workflow-detail.md](references/workflow-detail.md). **Anchor reader:** before the create-offer, read `feature.md`'s `Ticket:` — blank/placeholder → offer; valid sanitized ref → ticket path, re-resolve fork identities, re-hydrate cache. Detail: [references/ticket-creation.md](references/ticket-creation.md).
