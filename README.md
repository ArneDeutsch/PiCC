...
the current agent policy; `/doctor` gives an explicit project compatibility report; `/mcp` shows
bounded read-only MCP server status; `/usage` reports a per-subagent token/cost breakdown; `/quota` reports provider quota
headers; `alt+a` opens the
